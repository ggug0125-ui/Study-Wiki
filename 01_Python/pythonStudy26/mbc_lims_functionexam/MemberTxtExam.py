...
#   0     1      2   3     4
# members[1][3] -> 두번째 회원의 권한이 된다
# members 구조:
# [아이디, 비밀번호, 이름, 권한, 활성화 여부]
# 변수 : uid,  pw,     name,   admin, active
...
def save_members():
    ...
    with open(FILE_NAME, "w", encoding="utf-8") as f:  #f는 열린 파일
        #   상수변하지않아  쓰기(덮어쓰기)  한글처리용(유니코드)  파일객체(파일이 들어가있는 변수)
        #                 a = 추가용
...
def load_members(): # 텍스트 파일을 전체 불러와 리스트로 만듬~!!왜??? 중간수정이 안대~
    ...
    with open(FILE_NAME, "r", encoding="utf-8") as f:
        ...
        for line in f: # f 파일내용을 한줄씩 line 변수에 넣음
            ...
            #print("-------------------------")
            #변조후 데이터를 확인해보면 모두다 문자열로 취급됨
            #문자열 True를 불리언(True/False)타입으로 변환
            data[4] = True if data[4] == "True" else False # "True" 따옴표 없애라
            #받은변수  넣을값     data 4번째 문자열 True이면
            #                                  아니면 False
            # if data[4] == "True" :
            #    data[4] = True
            #else :
...
def member_login():
    ...
    # 없습니다.
    # idx: 1
    # member: ['lhj', '임효정', '0000', 'manager', True]
    # member[0]: lhj
    # member[1]: 임효정
    # 찾는
    # 아이디가
