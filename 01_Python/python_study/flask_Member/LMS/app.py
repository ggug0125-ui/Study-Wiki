...
@app.route("/")
def main():     # 메인 게시판 미리보기
    notices = BoardService.notice_preview(limit=5)
...
def board_list():  # 게시글 전체보기
    board_type = request.args.get("type", "all")  # ?type=notice/free/qna
    keyword = (request.args.get("q") or "").strip()  # ?q=검색어

    page = int(request.args.get("page", 1))  # ?page=1
    size = int(request.args.get("size", 10))  # ?size=10

    result = BoardService.list_paged(board_type=board_type, keyword=keyword, page=page, size=size)
    # result = {"items": [...], "page":..., "size":..., "total":..., "pages":...}
...
def board_view(board_id):                   # 게시글 보기
    if "user_id" not in session:
        return "<script>alert('로그인이 필요합니다.');location.href='/login';</script>"
...
@app.post("/board/pin/toggle/<int:board_id>")
def board_pin_toggle(board_id):                         # 상단 고정

    # 로그인 체크
    if "user_id" not in session:
        return "<script>alert('로그인이 필요합니다');history.back();</script>"

    # 관리자만 가능 (원하면 작성자도 허용 가능)
    ...
    try:
        with conn.cursor() as cursor:
            ...
            # 현재 상태 확인
            cursor.execute(
                "SELECT is_pinned FROM boards WHERE id=%s",
                (board_id,)
            )
            row = cursor.fetchone()

            if not row:
                return "<script>alert('게시글 없음');history.back();</script>"

            new_value = 0 if row["is_pinned"] == 1 else 1
    ...
    return redirect(request.referrer or "/board/list")

@app.post("/board/comment/add")
def board_comment_add():                       # 댓글 기능
    if "user_id" not in session:
        return "<script>alert('로그인이 필요합니다.');history.back();</script>"
...
def board_like_toggle():                           # 좋아요 조회수 증가
    if "user_id" not in session:
...
def board_edit(board_id):                           # 게시글 수정

    # 로그인 체크
    if 'user_id' not in session:
        return redirect(url_for('login'))
    board = BoardService.get(board_id)
    if not board:
        return "<script>alert('게시글이 없습니다.');history.back();</script>"
    # ✅ 작성자만 수정 가능
    ...
    BoardService.update(board_id, title, content)

    return redirect(url_for("board_view", board_id=board_id))

@app.post("/board/delete/<int:board_id>")
def board_delete(board_id):                           # 게시글 삭제
    # 로그인 체크
    if 'user_id' not in session:
        return redirect(url_for('login'))
    ...
    # 권한 체크: 작성자 or 관리자
    if board.member_id != session.get('user_id') and session.get('user_role') != 'admin':
        return "<script>alert('삭제 권한이 없습니다.');history.back();</script>"

    BoardService.delete(board_id)

    # 삭제 후 목록으로
    return redirect(url_for('board_list', type=board.board_type))
