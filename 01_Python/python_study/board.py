...
while run:
    ...
    if select == "1": #키보드로 입력한 값이 1이면?
        ...
        print("[게시글 전체 목록 출력]") #for 문돌리기 맥스가 있는배열
        # 게시글 리스트 보기용 코드 추가
        print("----------------------------")
        ...
        print("게시글 자세히 보기")  # 게시물 번호로 보기
        # 게시글 자세히 보기 코드 추가

        #보고싶은 게시글 번호를 받아야함
        bno = int(input("게시글 번호 : "))

        if bno in board_no: #등록된 게시글 확인 있는지없는지
            print("게시글을 찾았습니다")
