...
class PostService:
    ...
    def save_post(member_id, title, content, files=None, upload_folder='uploads/'):
        ...
        try:
            with conn.cursor() as cursor:
                ...
                post_id = cursor.lastrowid  # lastrowid 게시글의 번호
                # 파일첨부테이블에 들어갈 내용
    ...
    @staticmethod
    def delete_post(post_id, upload_folder='uploads/'): # post_id 게시글 번호
        """게시글 및 관련 실제 파일 삭제"""
        conn = Session.get_connection()
        try:
            with conn.cursor() as cursor:
                ...
                # 2. 서버에서 실제 파일 삭제
                for f in files:
                    file_path = os.path.join(upload_folder, f['save_name']) # 저장된 파일이름
                    if os.path.exists(file_path):
                        os.remove(file_path) # 실제 하드에서 삭제 진행

                # 3. 게시글 삭제 (DB 외래키 ON DELETE CASCADE 설정 덕분에 attachments도 자동 삭제됨)
                # 삭제시 cascade 옵션을 하지않으면 자식테이블 데이터를 먼저 삭제후 부모테이블 데이터를 삭제
