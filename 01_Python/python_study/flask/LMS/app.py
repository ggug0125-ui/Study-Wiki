...
##################################### 파일 게시판 CRUD  ###############################################################
 ...
 ## 5. 파일명 중복방지용 코드를 활용하겠다.
 ## 6. DB에서 부모 객체가 삭제되면 자식객체도 삭제되게 cascade 처리함!

UPLOAD_FOLDER = 'uploads/'
# 폴더가 없으면 자동 생성
if not os.path.exists(UPLOAD_FOLDER): # import os 상단 추가  # 폴더가 없으면 만들어라
...
# 1 bit -> 0, 1
# 1 byte -> 8bit 를 1 byte라 한다 (0~255) 256개 값을 가지고있다
# 1KB - > 1024byte
# 1MB -> 1024Kbyte
# 1GB -> 1024Mbyte
# 1TB -> 1024Gbyte
...
def filesboard_write(): # 서비스파일에 save_post 여기랑 같이확인해~!!
    if 'user_id' not in session:
        return redirect(url_for('login'))
    if request.method == 'POST':
        ...
        # 파일처리시 html에 필수코드 : enctype="multipart/form-data"
...
def download_file(filename):
    ...
    # from flask import send_from_directory 필수              # 다운로드 상단 표시
...
def filesboard_delete(post_id):
    ...
    # _은 리턴값을 사용하지 않겠다 라는 관례적인 표현 (_) 사용하지 않는 변수 (넘어온값이 두개야~ 하나만 사용할때는  _ 처리)
